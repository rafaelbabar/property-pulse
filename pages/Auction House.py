...
def parse_min_bid(min_bid):
    # Ensure that the price is stripped of non-numeric characters and converted to a number
    min_bid_value = ''.join(filter(str.isdigit, min_bid))  # Extract only digits
    return float(min_bid_value) if min_bid_value else 0.0  # Default to 0 if no valid number
